...
class DateModel:
    ...
    def _format_date(self) -> str:
        ...
        if self.input_type == 'date' or self.input_type == 'empty':
            validated_date = self._validate_date()
            unix_date = calendar.timegm(validated_date.timetuple()) # generates unix timestamp
    ...
    def date_as_dict(self):
        ...
        """
        if self.input_type != 'date' and self.input_type != 'empty':
            return {"unix": int(self.date_str), "utc": self.get_date_from_timestamp}
        elif self.input_type == 'empty':
        ...
        if input != None:
            if input.group() == '':
                ...
                return 'date'
                    ...
                    return 'seconds'
            elif len(input.group()) >= 12 and len(input.group()) <= 13:
                    return 'milliseconds'
