...
class TestBlueprintApi:
    ...
    def test_blueprint_api_with_wrong_http_method(self, client):
        """
        GIVEN a flask app is configured for testing
        WHEN endpoint with date param '/api/<DATE>' is requested with wrong HTTP Method
        THEN checks if http status code is equal to 200
        """
        response = client.post("/api/2023-01-01")
    ...
    def test_blueprint_api_with_timestamp_input_equals_expected_output(self, client):
        ...
        OBS: FLASK SORT THE JSON KEYS
        """
        expected_output = {
            "timestamp_format": "seconds",
            "unix": 1673913600,
            "utc": "Tue, 17 Jan 2023 00:00:00 GMT",
        }

        response = client.get("/api/1673913600")
        assert response.json == expected_output

    def test_blueprint_api_with_invalid_input_equals_expected_error_output(
        self, client
    ):
        """
        GIVEN a flask app is configured for testing
